...
#
# Launches `ghcr.io/groupdocs-redaction/redaction-net-mcp` as an interactive
# MCP server with the project's sample-docs/ folder mounted at /data. Use this
# for ad-hoc smoke testing of the published container artifact — the existing
...
#                         stdin and verify all five tools are advertised. Returns
#                         0 on success.
#
# Prerequisites:
...
#                           advertise all five tools.
#   --license PATH          Mount a GroupDocs license file (removes watermarks;
#                           without it the server runs in evaluation mode and all
#                           tools still work)
...
IMAGE_REPO="ghcr.io/groupdocs-redaction/redaction-net-mcp"
...
# nothing to /data when only running get_document_info / redact_text without
# saving to the same mount.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — output will be watermark-free"
else
    log_info "No license — output will be watermarked (all tools still work in evaluation mode)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    for tool in redact_text erase_metadata redact_annotations redact_image_area get_document_info; do
        if ! echo "$response" | grep -q "\"$tool\""; then
            log_error "Tool not found in response: $tool"
            all_tools_found=false
        fi
    done

    if [ "$all_tools_found" = true ]; then
        log_success "All five tools advertised — smoke test PASSED"
        exit 0
    else

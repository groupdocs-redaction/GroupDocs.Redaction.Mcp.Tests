...
main() {
    echo ""
    echo "╔════════════════════════════════════════════════════╗"
    echo "║  GroupDocs.Redaction.Mcp — Setup Verification     ║"
    echo "╚════════════════════════════════════════════════════╝"
    echo ""
    ...
    # Check test project
    if [ -d "$PROJECT_ROOT/src/GroupDocs.Redaction.Mcp.Tests" ]; then
        print_pass "Test project found"
    else
        print_fail "Test project not found: $PROJECT_ROOT/src/GroupDocs.Redaction.Mcp.Tests"
    fi

    # Check csproj
    if [ -f "$PROJECT_ROOT/src/GroupDocs.Redaction.Mcp.Tests/GroupDocs.Redaction.Mcp.Tests.csproj" ]; then
    ...
    if [ -d "$PROJECT_ROOT/sample-docs" ]; then
        ...
        if [ "$doc_count" -gt 0 ]; then
            ...
            print_warn "sample-docs exists but is empty (tests may fail without sample documents)"
        fi
    else
    ...
    local mcp_image="ghcr.io/groupdocs-redaction/redaction-net-mcp:latest"

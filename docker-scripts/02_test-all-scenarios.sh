#!/bin/bash
################################################################################
# GroupDocs.Redaction.Mcp — Integration Test Runner (local)
#
...
#   - RedactText (DOCX/PDF text redaction)
#   - EraseMetadata (document metadata erasure)
#   - RedactAnnotations (annotation redaction / delete-all)
#   - RedactImageArea (rectangular region covering)
#   - GetDocumentInfo (read-only file info)
#   - ErrorHandling (unknown files, corrupted bytes, password param)
#
# Prerequisites:
#   - .NET 10 SDK
#   - sample-docs folder with test fixtures
#   - GROUPDOCS_LICENSE_PATH env var (optional, removes watermarks from output)
#
# Usage:
...
#                           most recent stable release. Pin (e.g. "26.5.1") for
#                           reproducible / shared / CI runs.
#   --filter PATTERN        Run only tests matching pattern (e.g., RedactText)
...
#   ./02_test-all-scenarios.sh --version 26.5.1 --filter RedactText
...
TEST_PROJECT_DIR="$PROJECT_ROOT/src/GroupDocs.Redaction.Mcp.Tests"
...
main() {
    ...
    echo ""
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║  GroupDocs.Redaction.Mcp — Integration Test Runner (local)║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo ""

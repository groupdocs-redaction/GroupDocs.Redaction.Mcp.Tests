...
public class ToolDiscoveryTests : McpServerTestBase
{
    ...
    [Fact]
    public void ServerInfo_AdvertisesGroupDocsRedactionMcp()
    {
        var info = _fixture.Client.ServerInfo;

        Assert.NotNull(info);
        Assert.Equal("GroupDocs.Redaction.Mcp", info!.Name);
    ...
    [Fact]
    public async Task ListTools_ExposesAllFiveRedactionTools()
    {
        ...
        Assert.NotNull(catalog.RedactText);
        Assert.NotNull(catalog.EraseMetadata);
        Assert.NotNull(catalog.RedactAnnotations);
        Assert.NotNull(catalog.RedactImageArea);

...
public class ErrorHandlingTests : McpServerTestBase
{
    ...
    public async Task GetDocumentInfo_UnknownFile_ReturnsErrorOrAvailableFilesHint()
    {
        ...
        // The tools promise to resolve from storage and return available files when
        // a name isn't found.
    ...
    [Fact]
    public async Task RedactText_CorruptedFile_DoesNotCrashServer()
    {
        ...
        // Server should either return a tool error (Pitfall #18 surfaces it as a
        // "Text redaction failed for ..." message) or throw an MCP error — either
        // way, it must stay up and continue answering subsequent requests.
        try
        {
            var response = await _fixture.Client.CallToolAsync(
    ...
    [Fact]
    public async Task GetDocumentInfo_PasswordParameter_IsAccepted()
    {
        ...
        // Wrong password on a non-protected file — the tool should treat it as a
        // no-op or return a clear error, but must accept the `password` parameter
        // without rejecting the schema.
        var catalog = await ToolCatalog.LoadAsync(_fixture.Client);

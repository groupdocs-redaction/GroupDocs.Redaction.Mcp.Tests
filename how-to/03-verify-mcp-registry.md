...
The `GroupDocs.Redaction.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Redaction.Mcp](https://www.nuget.org/packages/GroupDocs.Redaction.Mcp)
...
- `command: "dnx"`
- `args: ["GroupDocs.Redaction.Mcp@26.5.0", "--yes"]`
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Redaction.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-redaction/groupdocs-redaction-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Redaction.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-redaction/GroupDocs.Redaction.Mcp" }
}
```
...
3. Search for `groupdocs-redaction` or `redaction`.
4. The package should appear with the description from `server.json`:
   *"MCP server exposing GroupDocs.Redaction for .NET — redact text by regex, erase metadata, redact or delete annotations, and cover image areas in documents via AI agents."*
...
- `dnx` successfully resolves `GroupDocs.Redaction.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-redaction"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsRedactionMcp` — the server
  reports `serverInfo.name == "GroupDocs.Redaction.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesAllFiveTools` — all five MCP tools
  are advertised.

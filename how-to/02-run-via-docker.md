...
- `ghcr.io/groupdocs-redaction/redaction-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/redaction-net-mcp` — Docker Hub (mirror)

Each release is tagged with its version (`:26.5.1`) and `:latest`.
...
Each release pushes both `:<version>` (e.g. `:26.5.1`) and updates `:latest` to point at it. Pick the tag that matches how you want to handle upgrades:

```bash
docker pull ghcr.io/groupdocs-redaction/redaction-net-mcp:26.5.1   # pinned to exact version
docker pull ghcr.io/groupdocs-redaction/redaction-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-redaction/redaction-net-mcp:latest
...
|---|---|---|
| `:26.5.1` | Locked to that release. No surprise upgrades. | Committed configs, CI, shared team setups. |
...
```bash
docker inspect ghcr.io/groupdocs-redaction/redaction-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-redaction/redaction-net-mcp:26.5.1 2>/dev/null
```
...
and `get_document_info` with their descriptions and input schemas.
...
services:
  groupdocs-redaction-mcp:
    image: ghcr.io/groupdocs-redaction/redaction-net-mcp:26.5.1
    stdin_open: true
    tty: false
    environment:
...
Without the license, all redaction tools still succeed — output files are
produced but will contain a watermark overlay
...
docker inspect ghcr.io/groupdocs-redaction/redaction-net-mcp:26.5.1 \
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-redaction/redaction-net-mcp](https://github.com/groupdocs-redaction/GroupDocs.Redaction.Mcp/pkgs/container/redaction-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-redaction/redaction-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |

...
2. Open Copilot chat.
3. Click the tools / MCP icon — `groupdocs-redaction` should appear with all
   five tools listed: `redact_text`, `erase_metadata`, `redact_annotations`,
...
The [NuGet.org package page](https://www.nuget.org/packages/GroupDocs.Redaction.Mcp)
...
```
@workspace redact all email addresses from /docs/report.pdf

Use redact_text with pattern \b\d{16}\b to mask card numbers in invoice.docx.

Erase all metadata fields from all DOCX files under /uploads.

Cover the header region (x=0, y=0, width=595, height=80) on every page of brief.pdf.
```
...
Without a license, all tools still work in evaluation mode — output will
contain a watermark (see

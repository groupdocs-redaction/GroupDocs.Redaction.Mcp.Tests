...
Two install routes from [`nuget.org/packages/GroupDocs.Redaction.Mcp`](https://www.nuget.org/packages/GroupDocs.Redaction.Mcp):
...
```bash
dnx GroupDocs.Redaction.Mcp --yes                # latest stable, refreshed every run
dnx GroupDocs.Redaction.Mcp --prerelease --yes   # latest including pre-releases
```
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.redaction.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.redaction.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Redaction.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Redaction.Mcp@26.7.0 --yes
```

You should see two JSON-RPC responses. The second contains all five tools:
...
```bash
dotnet tool update -g GroupDocs.Redaction.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Redaction.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-redaction-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-redaction-mcp.exe` (Windows). Make sure
...
|---|---|---|
| `GROUPDOCS_MCP_STORAGE_PATH` | Folder the server reads input from and writes redacted copies to | current working directory |
| `GROUPDOCS_MCP_OUTPUT_PATH` | Optional — route output files to a different folder | same as storage |
| `GROUPDOCS_LICENSE_PATH` | Path to `GroupDocs.Total.lic` — removes evaluation watermarks | *(evaluation mode)* |
...
All five tools work in evaluation mode — `GroupDocs.Redaction.Save()` succeeds
without a license. Output files are produced but contain a watermark overlay.

Get a license from [purchase.groupdocs.com](https://purchase.groupdocs.com/).
Point `GROUPDOCS_LICENSE_PATH` at the `.lic` file to produce clean, watermark-free
output.
...
```text
initialize response → serverInfo: { name: "GroupDocs.Redaction.Mcp", version: "26.7.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. All tools still work; output will contain a watermark. Set the path to remove watermarks. |
